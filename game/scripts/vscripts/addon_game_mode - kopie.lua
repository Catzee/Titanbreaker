...
"pve_temple_flamepool_run",
    ...
    self.act_kills_max = {5,7,9,9,14,9,4,8,30,2,11,14,3}
    self.act11_bosses = {}
    ...
    --self.hero_spells[index]["RegrowthFeral"] = 30
	...
	GameRules:SetHideKillMessageHeaders( true )
	GameRules:SetUseUniversalShopMode( false ) --false!
...
function RemoveAllCosmetics( hero )
    ...
    for slot=1, COverthrowGameMode.mythic_slots do
        if hero and hero.cosmetic_rewards and hero.cosmetic_rewards[slot] then
            hero.cosmetic_rewards[slot]:Destroy()
...
function COverthrowGameMode:OnThink()
    ...
    --test merge boss
    --[[
    if self.act_1_final_boss then
        self.act_1_final_boss.act = math.random(1,2)
        for i=1,25 do
            COverthrowGameMode:DropTempleItem(self.act_1_final_boss, 100, 1, 0, false)
        end
    end 
    ...
    --if HeroList:GetAllHeroes()[1] then
        --COverthrowGameMode:DropTempleItem( HeroList:GetAllHeroes()[1], 100, 2, 8, true )
    --end
	...
	if not self.allplayershavehero and GameRules:State_Get() == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
		...
		if allin then
            ...
            Timers:CreateTimer(time_to_choose,function()
                ...
                CustomGameEventManager:Send_ServerToAllClients( "report_drop_chance", {value = self.jungledifficulty, boss = 0} )
                self.difficulty_is_set = true
                local all = HeroList:GetAllHeroes()
                all = #all
                local interpolation_factor = (self.jungledifficulty-1) / 9
                if interpolation_factor > 1 then
                    interpolation_factor = 1
                end
                self.player_count_scale = (0.0 + 0.2 * all) * (1-interpolation_factor) + interpolation_factor
                local reduction = 1 - self.player_count_scale
                if reduction > 0.0 then
                    local msg = ""..tostring(all).." Player(s) detected. Monster Damage and Health reduced by "..tostring(reduction*100).."%. Less effective the higher the Monster Level is."
                    Notifications:BottomToAll({text=msg, duration=10, style={color="orange"}})
                end
                --unlock tps
    ...
    if COverthrowGameMode.act13_event_in_progress or (mogushan_boss and not mogushan_boss:IsNull() and mogushan_boss:GetHealth() / mogushan_boss:GetMaxHealth() < 1) then
        ...
        for i=1, #all do
            hero = all[i]
            if hero and ((hero:GetAbsOrigin() - center):Length() > max_distance or (COverthrowGameMode.act13_event_in_progress and hero:GetAbsOrigin()[1] < 12900)) and hero.combat_system_ability then
	...
	if self.junglemode then
       ...
       or steamid == 100437970 or steamid == 247058932 or steamid == 433543739 or steamid == 207219111 or steamid == 138250514 or steamid == 100678658 or steamid == 422948593 or steamid == 365686150 or steamid == 366222608
        ...
        or steamid == 188237126 or steamid == 197202228 then -- or steamid == 39460724 then
				--GameRules:SetGameWinner(DOTA_TEAM_NEUTRALS)
				hero.cheater = true
			end
		end
		...
		for i=1, #all do
			...
			if hero and hero.inventory then
                ...
                for index=0,6 do
                    local dmg = 0
                    local heal = 0
                    local name = hero:GetAbilityByIndex(index):GetAbilityName()
			...
			if self.farmtimer >= farminterval then
				...
				for i=1, 4 do
					if self.farm_units[i] and not self.farm_units[i]:IsNull() and self.farm_units[i]:IsAlive() then
						...
						local spawn
						if i <= 2 then
							spawn = CreateUnitByName("npc_dota_creature_city_wolf_1", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
						end
						if i >= 3 then
							spawn = CreateUnitByName("npc_dota_creature_city_wolf_2", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
						end
						spawn:SetHullRadius(30)
						spawn:GetAbilityByIndex(0):ApplyDataDrivenModifier(spawn, spawn, "modifier_phased", tab)
						spawn:CreatureLevelUp(scale)
			...
			if self.farmtimer_big >= farminterval_big then
				...
				for i=5, 10 do
					if self.farm_units[i] and not self.farm_units[i]:IsNull() and self.farm_units[i]:IsAlive() then
						...
						local spawn
						if i == 8 then
							spawn = CreateUnitByName("npc_dota_creature_city_kobold_boss", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
							spawn.drop_goldcore = 1
							spawn.camp_position = self.farmspot[i]
						end
						if i == 10 then
							spawn = CreateUnitByName("npc_dota_creature_city_frost_boss", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
							spawn.drop_frostcore = 1
						end
						if i == 9 then
							spawn = CreateUnitByName("npc_dota_creature_city_shadow_boss", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
							spawn.drop_shadowcore = 1
						end
						if i == 5 then
							spawn = CreateUnitByName("npc_dota_creature_city_iron_boss", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
							spawn.drop_ironcore = 1
						end
						if i == 7 then
							spawn = CreateUnitByName("npc_dota_creature_city_fire_boss", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
							spawn.drop_firecore = 1
						end
						if i == 6 then
							spawn = CreateUnitByName("npc_dota_creature_city_nature_boss", self.farmspot[i], true, nil, nil, DOTA_TEAM_NEUTRALS)
		...
		if self.RoundCountdown == 0 and not self.junglemode and (self.FarmMode == 0 or (self.RoundCounter ~= 4 and self.RoundCounter ~= 8 and self.RoundCounter ~= 12))  then
			...
			if self.PVEMode == 1 then
				...
				if self.currentBoss == 1 then
					boss = CreateUnitByName("npc_dota_creature_pveboss1", pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
...
function COverthrowGameMode:EndArena(winningteam)
	...
	--[[
	local allPets = Entities:FindAllByName("npc_dota_creature_waterelemental")
	print("pets found")
	print(allPets)
	print(allPets[1])
	for key,value in pairs(allPets) do 
		print("remove Pet")
		print(key) 
		print(value)
		value:Destroy()
...
function COverthrowGameMode:UnitAI(event)
    ...
    if boss:HasModifier("modifier_loot_goblin") and boss:GetHealth() / boss:GetMaxHealth() < 1 then
        ...
        boss.loot_goblin_timer = boss.loot_goblin_timer - 2
        DecreaseBuffStack({caster = boss, amount = 6, buff = "modifier_loot_hoarder_damage_reduction"})
    ...
    if boss:HasModifier("modifier_aggro_highest_hp") then
        target = self:AggroHighestHP(boss)
    end
            ...
            if not maniac_affix then
                if not boss:HasModifier("modifier_invisible") then
					...
					if ability and ability:GetCooldownTimeRemaining() <= 0.0 then
						...
						--print("cast correct "..boss.next_ability)
						--yes, cast it
						local order = 
						{
                            UnitIndex = boss:entindex(),
...
function COverthrowGameMode:PVEClearMobs()
	local all = HeroList:GetAllHeroes()
	local enemies = FindUnitsInRadius( DOTA_TEAM_NEUTRALS, Vector(0,0,0), all[1], FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false )
	--print(#enemies)
	if #enemies > 0 then
		for _,enemy in pairs(enemies) do
			if enemy ~= nil then
...
function COverthrowGameMode:PopulateDungeon(round)
	...
	for i=1, 2 do
		...
		if round == 4 then
			local unit
			unit = CreateUnitByName("pve_spider", p21, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,5,250,1)
			unit = CreateUnitByName("pve_spiderranged", p22, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,150,1)
			unit = CreateUnitByName("pve_spiderranged", p23, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,150,1)
			unit = CreateUnitByName("pve_spiderboss", p3, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,10,650,2)

			unit = CreateUnitByName("pve_spidertank", p41, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,5,200,1)
			unit = CreateUnitByName("pve_spiderranged", p42, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,150,1)
			unit = CreateUnitByName("pve_spiderranged", p43, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,150,1)
			unit = CreateUnitByName("pve_undyingboss", p5, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,11,750,2)

			unit = CreateUnitByName("pve_undyingtank", p61, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,15,250,1)
			unit = CreateUnitByName("pve_undyingtank", p62, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,15,250,1)
			unit = CreateUnitByName("pve_skeleboss", p7, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,18,750,2)
			unit.islastboss = 1
			--boss = CreateUnitByName("npc_dota_creature_pveboss1", q3, true, nil, nil, DOTA_TEAM_NEUTRALS )
			
		end

		if round == 8 then
			local unit
			unit = CreateUnitByName("pve_beartank", p21, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,180,1)
			unit = CreateUnitByName("pve_beartank", p22, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,180,1)
			unit = CreateUnitByName("pve_beartank", p23, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,180,1)
			unit = CreateUnitByName("pve_bearboss", p3, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,10,700,2)

			unit = CreateUnitByName("pve_beartank", p41, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,8,150,1)
			unit = CreateUnitByName("pve_bearstunner", p42, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,5,150,1)
			unit = CreateUnitByName("pve_bearstunner", p43, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,5,150,1)
			unit = CreateUnitByName("pve_bearboss2", p5, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,15,750,2)

			unit = CreateUnitByName("pve_mountaingiant", p61, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,60,225,1)
			unit = CreateUnitByName("pve_mountaingiant", p62, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,60,225,1)
			unit = CreateUnitByName("pve_bearboss3", p7, true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		if round == 12 then
			local unit
			unit = CreateUnitByName("pve_lizard", p21, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,9,100,1)
			unit = CreateUnitByName("pve_lizard", p22, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,9,100,1)
			unit = CreateUnitByName("pve_lizard", p23, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,9,100,1)
			unit = CreateUnitByName("pve_dragonboss", p3, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,10,700,3)

			unit = CreateUnitByName("pve_lizard2", p41, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,5,150,1)
			unit = CreateUnitByName("pve_lizard3", p42, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,18,75,1)
			unit = CreateUnitByName("pve_lizard3", p43, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,18,75,1)
			unit = CreateUnitByName("pve_dragonboss2", p5, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,13,700,3)

			unit = CreateUnitByName("pve_lizard", p61, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,20,130,1)
			unit = CreateUnitByName("pve_lizard", p62, true, nil, nil, DOTA_TEAM_NEUTRALS )
			self:SetPVEStats(unit,scale,20,130,1)
			unit = CreateUnitByName("pve_dragonboss3", p7, true, nil, nil, DOTA_TEAM_NEUTRALS )
...
function PVEItemDrop(event)
	...
	if event.caster.rewardchance then
		...
		if event.caster.sandevent then
			...
			if COverthrowGameMode.sandevent >= 5 then
                ...
                unit:GetAbilityByIndex(0):ApplyDataDrivenModifier(unit, unit, "modifier_invulnerable", {Duration = -1})
                --furycat
				unit = CreateUnitByName("temple_sand_boss_4", b4, true, nil, nil, DOTA_TEAM_NEUTRALS )
				unit.act = 2
                unit.act_specific_loot_factor = 3
				COverthrowGameMode:SetTempleStats(unit,COverthrowGameMode.jungledifficulty*1.1,100,12500,100, 0)
        ...
        if event.caster.templeboss_mogushan then
            ...
            if round < 30 then
                Notifications:BottomToAll({text="Round "..round.." defeated! The next round will start in 30 secs with 10% increased Monster Level.", duration=10, style={color="orange"}})
                ...
                Notifications:BottomToAll({text="Round "..round.." defeated! This was the last round, granting Items with 500x increased drop chances. Well done!", duration=10, style={color="orange"}})
            end
        end
        --[[
		if event.caster.templeboss_mogushan then
			...
			if (COverthrowGameMode.mogushankills % 2) == 0 and COverthrowGameMode.mogushankills >= 2 and COverthrowGameMode.mogushankills <= 18 then
				...
				Timers:CreateTimer(30, function()
					...
					unit = CreateUnitByName(bossname, c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
                    unit:SetForwardVector(Vector(0,-1,0))
					unit.act = 9
					...
					unit = CreateUnitByName(bossname2, c1+Vector(350,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
                    unit:SetForwardVector(Vector(0,-1,0))
			...
			if COverthrowGameMode.mogushankills == 24 then
				...
				Timers:CreateTimer(30, function()
					...
					unit = CreateUnitByName(bossname3, c1+Vector(-350,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
                    unit:SetForwardVector(Vector(0,-1,0))
					unit.chance_for_mogushan_affixes = 100
					unit.act = 9
					unit.exp_factor = 5
    ...
    if event.caster.unlock_improved_tp_act_11 then
      ...
      --COverthrowGameMode.teleporters[0].tp_active = true
		...
		if unit.templeboss_sandviper then
			local i = 2
			COverthrowGameMode.boss_kills_per_act[i] = COverthrowGameMode.boss_kills_per_act[i] + 1
			if COverthrowGameMode.boss_kills_per_act[i] == 6 then
				COverthrowGameMode:CreateActBoss(11,i)
			end
		end
		if unit.templeboss_wetlands then
			local i = 3
			COverthrowGameMode.boss_kills_per_act[i] = COverthrowGameMode.boss_kills_per_act[i] + 1
			if COverthrowGameMode.boss_kills_per_act[i] == 9 then
				COverthrowGameMode:CreateActBoss(11,i)
			end
		end
        ...
        if unit.act13_last_boss then
            ...
            --end
            for i=1,2 do
                local unit = CreateUnitByName("temple_chest", Entities:FindByName( nil, "act13_3" ):GetAbsOrigin() + Vector(-450+300*i,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        ...
        if not COverthrowGameMode.quest_act1andact2 and COverthrowGameMode.boss_kills_per_act[1] and COverthrowGameMode.boss_kills_per_act[1] >= 4 and COverthrowGameMode.boss_kills_per_act[2] and COverthrowGameMode.boss_kills_per_act[2] >= 5 then
...
function COverthrowGameMode:AggroHighest(caster)
	local all = HeroList:GetAllHeroes()
	local aggro = -1
	local unit
	for i=1, #all do
...
function COverthrowGameMode:AggroHighestHP(caster)
	...
	for i=1, #all do
		...
		if all[i]:IsHero() and all[i]:IsAlive() and not (all[i]:IsIllusion()) and (all[i]:GetAbsOrigin()-caster:GetAbsOrigin()):Length() <= 2500 and caster then
			...
			if h > hp and caster.aggrolist and caster.aggrolist[ownerid] and caster.aggrolist[ownerid] > 0 then
				unit = all[i]
				hp = h
			end
		end
	...
	if caster and caster.aggrolist and (source:IsHero() or source.owner) then
		...
		if source:IsHero() and source:GetPrimaryAttribute() == 0 then
            if source.super_aggro_tank and not event.ignore_super_aggro_tank then
                ...
                if level_tank_ability and level_tank_ability >= 4 then
                    local aggro_bonus_armor = 1 + source:GetPhysicalArmorValue() * 0.1
                    if aggro_bonus_armor < 1 then
                        aggro_bonus_armor = 1
                    end
                    local aggro_bonus_resist = 1 + source:GetMagicalArmorValue() * 100 * 0.2
                    if aggro_bonus_resist < 1 then
                        aggro_bonus_resist = 1
                    end
                    local aggro_bonus_hp = 1 + source:GetMaxHealth() / 1000
...
function COverthrowGameMode:OnNPCSpawned( keys )
 ...
 function COverthrowGameMode:OnHeroInGame(hero)
    --print("Hero spawned in game for first time -- " .. hero:GetUnitName())
    local heroName = hero:GetUnitName()
    local firstmsg = 120
    local secondmsg = 240
    local msgdur = 20
  ...
  if heroName == "npc_dota_hero_axe" or heroName == "npc_dota_hero_dragon_knight" or heroName == "npc_dota_hero_beastmaster" or heroName == "npc_dota_hero_bristleback" or heroName == "npc_dota_hero_ursa" then
       ...
       Timers:CreateTimer(firstmsg,function()
            if hero:GetLevel() < msglevelthreshold then
   ...
   if heroName == "npc_dota_hero_phantom_lancer" then
	...
	if heroName == "npc_dota_hero_skeleton_king" or heroName == "npc_dota_hero_sven" or heroName == "npc_dota_hero_pudge" or heroName == "npc_dota_hero_abaddon" or heroName == "npc_dota_hero_antimage" then
	    -- Add the spell
...
if heroName == "npc_dota_hero_dazzle" then
    ...
    if ability1 ~= nil then
        ...
        ability1:ApplyDataDrivenModifier(hero, hero, "modifier_sap", {Duration = 23})
    end
	...
	if killedUnit:IsRealHero() then
		...
		if self.junglemode then
			if killedUnit.pvelives and killedUnit.pvelives > 0 then
				...
				if (killedUnit:HasModifier("modifier_item_crit_pure_immortal3") and math.random(1,100) <= 20) or (killedUnit.talents and killedUnit.talents[63] and killedUnit.talents[63] > 0 and math.random(1,100) <= chance_not_lose_life) then
                    --print("no life lost")
                ...
                if killedUnit.combat_system_ability then
                    Timers:CreateTimer(2.1,function() 
                        killedUnit.combat_system_ability:ApplyDataDrivenModifier(killedUnit, killedUnit, "modifier_talent_invul", {duration = 10})
                    end)
                end
...
function GamblingPressedButton(event, args)
	...
	if item == 4 then
		...
		quality = 6
		artifact = true
		requirekills = nil --250
        require_monster_level = 5
        require_kiirion = true
        molten_forge = true
    ...
    if item >= 11 and item <= 17 then
        ...
        if slot <= 3 then
            token_required = "modifier_winter_token"
        else
            token_required = "modifier_jungle_token"
        end
    end
    ...
    if COverthrowGameMode.jungledifficulty < require_monster_level then
        Notifications:Top(hero:GetPlayerID(), {text="Monster Level requirements not met!", duration=5, style={color="red"}})
        return
    end
	...
	if hero then
		...
		if hero.buycooldown then
			Notifications:Top(hero:GetPlayerID(), {text="10 Seconds Cooldown!", duration=5, style={color="red"}})
			return
		end
...
function COverthrowGameMode:CraftSpecificArtifact( hero, quality, slot )
    if quality >= 12 then
        local weapon = "divi" .. slot .. "generate"
        if quality >= 13 or math.random(1,100) <= 3 then
...
function COverthrowGameMode:FilterDamage( filterTable )
	...
	--stop pet aa dmg
	if attacker:HasModifier("modifier_pet_system") or attacker:HasModifier("modifier_pet_system_grizzly") then
        return
    end
	...
	local newdamage = original_damage
    ...
    if victim.talents and victim.talents[33] and victim.talents[33] > 0 and not victim.resourcesystem then
        ...
        if hppermana > 0 then
            hppermana = hppermana * (1 + 0.4 * level)
        else
            hppermana = level * 4
        end
    end
    if hppermana > 0 then
        local absorbabledamage = newdamage * 0.75
    ...
    if victim.talents and victim.talents[19] and victim.talents[19] > 0 then
        local agi_block_range_melee = 0.15
        if not victim:IsRangedAttacker() then
            agi_block_range_melee = agi_block_range_melee * 1.5
        end
        local block_amount = agi_block_range_melee * victim.talents[19] * victim:GetAgility()
  ...
  --block agi moonglaive shield 45%
    ...
    if glaive_shield > 0 and victim.combat_system_ability then
        local block_factor = 0.3 + 0.1 * victim.talents[24]
    ...
    --block
    local block = 0
    local blockpath = victim:GetModifierStackCount("modifier_health_path", nil)
    if blockpath > 0 then
        --if victim:HasModifier("modifier_ancient_def_wolf") then
        --   blockpath = blockpath + 2
        --end 
        block = block + victim:GetMaxHealth() * blockpath / 200
    end
    if victim.talents and victim.talents[11] and victim.talents[11] > 0 then
        block = block + original_damage * 0.03 * victim.talents[11]
    end
	...
	if victim.talents and victim.combat_system_ability then
        if victim.talents[42] and victim.talents[42] > 0 and not victim:HasModifier("modifier_talent42cd") then
          ...
          if lifeafterdamage < 1 and not IsImmortal(victim) then
            ...
            victim.combat_system_ability:ApplyDataDrivenModifier(victim, victim, "modifier_talent42cd", { Duration = 120})
            victim.combat_system_ability:ApplyDataDrivenModifier(victim, victim, "modifier_talent_invul", { Duration = 1 * victim.talents[42] })
...
function COverthrowGameMode:SaveChar(hero)
  ...
  local steamid = tostring(PlayerResource:GetSteamAccountID(playerid))
		...
		if not (PlayerResource:GetConnectionState(playerid) == DOTA_CONNECTION_STATE_DISCONNECTED or PlayerResource:GetConnectionState(playerid) == DOTA_CONNECTION_STATE_ABANDONED) then
			...
			--end
			--print(playername)
			local xp = tostring(hero:GetCurrentXP())
            if hero.reset_hero_level_on_save then
                xp = "1"
            end
            local gold_to_save = hero:GetGold()
            local cost_for_path_reset = 1000
			...
			bosskills = math.floor(bosskills*COverthrowGameMode.jungledifficulty)
			...
			CustomGameEventManager:Send_ServerToPlayer(player, "savechar", { hero = heroname, gold = goldsave, slot1 = items[0], slot2 = items[1], slot3 = items[2], slot4 = items[3], slot5 = items[4], slot6 = items[5], 
				slot7 = items[6], slot8 = items[7], slot9 = items[8], slot10 = items[9], slot11 = items[10], slot12 = items[11], slot13 = items[12], slot14 = items[13], slot15 = items[14], pn = playername, playerid = steamid, 
				...
				bestdps = dps, besthps = hps, ability1 = temple_abilities[1], ability2 = temple_abilities[2], ability3 = temple_abilities[3], ability4 = temple_abilities[4], ability5 = temple_abilities[5]
             } )
...
function LoadMyChar(caster)
	...
	local steamid = tostring(PlayerResource:GetSteamAccountID(playerid))
	--if GameRules:PlayerHasCustomGameHostPrivileges(player) then
	--print("loading char lua " .. hero:GetName())
	local heroname = hero:GetName()
	heroname = string.sub(heroname, 15)
	local hc = 0
	if GetMapName() == "tod_hardcore" then
	...
	CustomGameEventManager:Send_ServerToPlayer(player, "loadchar", { hero = heroname, playerid = steamid, hardcore = hc} )
...
function TalentPointsSpentInTree(hero, tree)
    ...
    if tree <= 4 then
        ...
        --new tree
        for i=15*(tree-1)+1, 15*tree-1 do
            if hero.talents_clicked[i] then
                sum = sum + hero.talents_clicked[i]
            end
        end
...
function TalentPressedButton(clicked, args)
	...
	if hero then
		if hero.talentpoints and hero.talentpoints > 0 and hero.talents_clicked and hero.talents_clicked[nr] and hero.talents_clicked[nr] < 3 and TalentPointAllowed(hero, nr) then
            ...
            local threshold = 700
            if distance <= threshold or not clicked then
...
function SummonPathChampion(monster_type, hero)
    ...
    local unit = CreateUnitByName(monster, position, true, nil, nil, DOTA_TEAM_NEUTRALS )
    if monster_type == 1 or monster_type == 4 then
            ...
            if id and steamid == id and not hero.auto_loaded then
				...
				if hero and hero.ArenaControl then
                    ...
                    hero.ArenaControl:AddAbility("temple_teleport"):SetLevel(1)
  						...
  						--if data[j+12] and data[j+12] ~= "no" then
  						for x = 1, self.mythic_slots do
  							hero.inventory[x][1] = data[j+12+4*(x-1)+3]
  							--print(hero.inventory[x][1])
  							--print(hero.inventory[x][1])
  							for y = 1, 3 do
  								if data[j+12+4*(x-1)+y+3] then
  									hero.inventory[x][y+4] = tonumber(data[j+12+4*(x-1)+y+3])
  								else
  									hero.inventory[x][y+4] = 0
  								end
  						...
  						--gold
  						local goldindex = j+16+self.mythic_slots*4+3
  						local bankindex = goldindex-1
              ...
              for temple_abil = 1, 5 do
                  if hero and hero.temple_class and data[goldindex+temple_abil] then
                      local ability_name = data[goldindex+temple_abil]
                      if ability_name and ability_name ~= "no" and ability_name ~= "empty_spell1" and ability_name ~= "empty_spell2" and ability_name ~= "empty_spell3" and ability_name ~= "empty_spell4" and ability_name ~= "empty_spell5" then
  						...
  						if temple_version == 2 and bank_slots > 0 then
  							local sharedindex = j+12+self.mythic_slots*4
                ...
                for m=1, count do
                  filler[m] = hero:AddItemByName( "item_dmg1" )
  			    		...
  			    		--add 3-6 hero bank items
  			    		--print("personal bank")
                         for n=j+8, j+7+bank_slots-3 do
  							--print(data[n])
  							--print("bank items")
  						...
  						--are we premium?
  						if bank_slots >= 9 then
  							local count2 = 6 - GetNumItemsStashAndBackpack(hero)
                local filler2 = {}
                for m2=1, count2 do
                  filler2[m2] = hero:AddItemByName( "item_dmg2" )
  				    			--print("added filler backpack and half stash inv")
  				    			--print(GetSlotItemIsIn(hero, filler2[m2]))
  				    	end

  							--print("shared bank")
  							for n=sharedindex+3, sharedindex-7+bank_slots+3 do
  								if data[n] and data[n] ~= "no" then
  									--print(data[n])
  									local newitem = hero:AddItemByName("item_"..data[n])
              ...
              --print(data[46])
                            Timers:CreateTimer(0.25,function() --wait a moment for path talent rings to update
                                if true or GetMapName() == "tod_hardcore" then
                                    local pathpoints = data[25+self.mythic_slots*4]  --with 7 artifact slots 49+4
                                    --print(pathpoints)
...
function COverthrowGameMode:SpawnStormCrowPatrols()
    ...
    --patrol
    for i=1, 3 do
        local unit = CreateUnitByName("pve_storm_crow", Vector(0,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.chance_for_champion = 100
        if i == 1 then
        ...
        elseif i == 2 then
            unit:AddAbility("pve_affix_runner"):SetLevel(1)
            unit.patrolling = 16
            ...
            unit.patrolling = 17
            self:SetTempleStats(unit,COverthrowGameMode.jungledifficulty * 1.5,200,2000,20,0)
        end
    end
end
...
function COverthrowGameMode:SpawnTreasureChest(pos, health_percent)
    ...
    if roll == 1 then
        ...
        unit = CreateUnitByName("temple_chest", pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.loot_goblin_flee_mode_straight = true
    end
    if roll == 2 then
        --random movement
        unit = CreateUnitByName("temple_chest_2", pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.loot_goblin_flee_mode_changing = true
        unit.loot_goblin_artifact_factor = 5
    end
    if roll == 3 then
        --runs straight, harder, 10% faster, gold drop
        unit = CreateUnitByName("temple_chest_3", pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.loot_goblin_flee_mode_straight = true
...
function COverthrowGameMode:PopulateTemple(zone)
	...
	local scale = 1.0 + ((zone-1)*0.25) --each zone 25% harder
	if scale < 1.0 then
		scale = 1.0
	end
	--scale = 0.1 --todo
	...
	scale = COverthrowGameMode.jungledifficulty
	...
	if zone == 1 then
		...
		--camp 1
		unit = CreateUnitByName("temple_wolf_rage", c1+Vector(-500,150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        ...
        --unit:AddAbility("pve_greedy_saw"):SetLevel(1)
        ...
        --unit:AddAbility("pve_affix_jumpstomp"):SetLevel(1)
        --unit:AddAbility("pve_temple_rockthrow"):SetLevel(1)
    ...
    for i=1, horde_factor do
        ...
        -- new camp 2
		unit = CreateUnitByName("temple_wolf_rage", c2new, true, nil, nil, DOTA_TEAM_NEUTRALS )
        --unit:AddAbility("pve_act_affix_lastwill"):SetLevel(1)
        --unit.chance_for_champion = 10
        ...
        unit:SetForwardVector(Vector(0,1,0))
        unit = CreateUnitByName("temple_wolf_rage", c2new+Vector(-250,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        --unit:AddAbility("pve_act_affix_lastwill"):SetLevel(1)
        unit.chance_for_champion = 10
        ...
        --camp 2.5
        unit = CreateUnitByName("temple_wolf_rage", c2new+Vector(1500,1000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.chance_for_champion = 10
        unit.exp_factor = 2
        ...
        --camp 3
        unit = CreateUnitByName("temple_wolf_rage", c3new, true, nil, nil, DOTA_TEAM_NEUTRALS )
        --unit.chance_for_champion = 10
		...
		unit = CreateUnitByName("temple_wolf_rage", c2+Vector(125,125,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:SetRenderColor(255, 255, 255)
		...
		unit = CreateUnitByName("temple_wolf_rage", c2, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 7
		...
		self:SetTempleStats(unit,scale,50*act_1_dmg_factor,250,4,0*affixinact1)
		--camp 3
		unit = CreateUnitByName("temple_wolf_brown", c3+Vector(150,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--camp 4
		unit = CreateUnitByName("temple_wolf_brown", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		unit = CreateUnitByName("temple_wolf_brown", c4+Vector(-125,-125,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125*act_1_dmg_factor,400,10,1*affixinact1)
		--patrol
		unit = CreateUnitByName("temple_wolf_brown", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 15
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,175*act_1_dmg_factor,1000,20,2*affixinact1)

		--camp 5
		unit = CreateUnitByName("temple_wolf_brown", c5, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,100*act_1_dmg_factor,650,10,1*affixinact1)
		unit:SetModelScale(0.9)
		unit = CreateUnitByName("temple_wolf_brown", c5+Vector(-125,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,100*act_1_dmg_factor,650,10,1*affixinact1)
		unit:SetModelScale(0.9)
    end
        ...
        --traps
        if scale > 1 then
            unit = CreateUnitByName("temple_spike_trap", c5+Vector(-500,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
            unit.exp_factor = 5
            self:SetTempleStats(unit,scale,750,100,0,0)
            unit = CreateUnitByName("temple_spike_trap", c6+Vector(550,750,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
            unit.exp_factor = 5
            self:SetTempleStats(unit,scale,750,100,0,0)
        end

		--camp 6
		unit = CreateUnitByName("temple_wolf_brown", c6, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_shadow_claw"):SetLevel(1)
		self:SetTempleStats(unit,scale,65*act_1_dmg_factor,600,4,0)
		unit = CreateUnitByName("temple_wolf_brown", c6+Vector(-100,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--boss 1
		unit = CreateUnitByName("temple_wolf_boss_1", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--boss 2
		unit = CreateUnitByName("temple_wolf_boss_2", b2, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		if self.jungledifficulty > 1.0 then
			unit:AddAbility("pve_temple_wolf_armor_strikes")
		end
		...
		unit.templeboss_wolf = 1
		--boss 3
		unit = CreateUnitByName("temple_wolf_boss_3", b3+Vector(0,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		unit = CreateUnitByName("temple_wolf_boss_3_2", b3+Vector(0,-250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--boss 4
		unit = CreateUnitByName("temple_wolf_boss_4", c3+Vector(1600,-1200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
    ...
    for i=1, horde_factor do
		--camp 1
		unit = CreateUnitByName("temple_sand_dustlord", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_slow_poison"):SetLevel(1)
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,225*act_dmg_factor,2500,20,2)
		--camp 1.5
		unit = CreateUnitByName("temple_sand_dustlord", c1+Vector(1050,2000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125*act_dmg_factor,500,7,1)
		unit:SetModelScale(0.85)
		unit = CreateUnitByName("temple_sand_dustlord", c1+Vector(1250,2000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_slow_poison"):SetLevel(1)
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,300*act_dmg_factor,750,7,1)
		unit:SetModelScale(1.5)
		unit = CreateUnitByName("temple_sand_dustlord", c1+Vector(1450,2000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125*act_dmg_factor,500,7,1)
		unit:SetModelScale(0.85)

        --traps
        unit = CreateUnitByName("temple_spike_trap", c1+Vector(1250+2200,2900+50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)

        --traps
        unit = CreateUnitByName("temple_missile_trap", c2+Vector(1200,750,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(0,-1,0))
		...
		--camp 2
		unit = CreateUnitByName("temple_sand_dustboar", c2+Vector(75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_slow_poison"):SetLevel(1)
		unit:SetModelScale(0.75)
		...
		self:SetTempleStats(unit,scale,90,400,5,1)
		--camp 3
		unit = CreateUnitByName("temple_sand_dustlord", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,100,1500,10,1)
		unit:SetModelScale(1.25)
		unit = CreateUnitByName("temple_sand_dustlord", c3+Vector(0,-250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 20
		self:SetTempleStats(unit,scale,180,1500,10,1)
		unit:SetModelScale(1.6)
		--camp 4
		unit = CreateUnitByName("temple_sand_dustboar", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		--unit.chance_for_champion = 10
		...
		--patrol
		unit = CreateUnitByName("temple_sand_dustboar", b1+Vector(0,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,150,850,10,1)
		unit:SetModelScale(0.9)
		unit = CreateUnitByName("temple_sand_dustboar", b2+Vector(0,-50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,150,850,10,1)
		unit:SetModelScale(0.9)
		...
		--camp 5
		unit = CreateUnitByName("temple_sand_stonegolem", c5, true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--camp 6
		unit = CreateUnitByName("temple_sand_dustgolem", c6, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_knockback"):SetLevel(1)
		self:SetTempleStats(unit,scale,185,1000,10,2)
		unit:SetModelScale(1.4)
		unit = CreateUnitByName("temple_sand_dustgolem", c6+Vector(0,-150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_knockback"):SetLevel(1)
		self:SetTempleStats(unit,scale,185,1000,10,2)
		unit:SetModelScale(1.4)
    end
		--camp 6
		unit = CreateUnitByName("temple_sand_dustgolem", c6+Vector(1200+math.random(1,1000),-500+math.random(1,1000),0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_knockback"):SetLevel(1)
        unit.exp_factor = 3
        ...
        --traps
        unit = CreateUnitByName("temple_missile_trap", c6+Vector(1500,1200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(-1,-1,0))
        ...
        unit.trap_element = "poison"
        unit.trap_timer = 4
        unit = CreateUnitByName("temple_missile_trap", c6+Vector(2900,250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(-1,-1,0))
        unit.trap_element = "poison"
        unit.trap_timer = 2
        unit = CreateUnitByName("temple_missile_trap", c6+Vector(2900,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        ...
        --traps
        unit = CreateUnitByName("temple_spike_trap", b1+Vector(0,-1500,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)

		--boss 1
		unit = CreateUnitByName("temple_sand_boss_3", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        ...
        unit:SetForwardVector(Vector(0,-1,0))
		--boss 2
		unit = CreateUnitByName("temple_sand_boss_2", b2, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		--boss 3
		unit = CreateUnitByName("temple_sand_boss_1", b3+Vector(50,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        ...
        unit:SetForwardVector(Vector(-1,0,0))
		--boss 4
		unit = CreateUnitByName("temple_sand_boss_5", b3+Vector(500,2400,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        ...
        unit:SetForwardVector(Vector(0,-1,0))
        --boss 5 deathgore
        unit = CreateUnitByName("temple_sand_bloodboar", b3+Vector(100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        ...
        unit:AddAbility("pve_lifesteal_low")
    ...
    for i=1, horde_factor do
		--camp 1
		unit = CreateUnitByName("temple_fungal_spawn", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,250,2000,20,2)
		unit:SetModelScale(1.75)
		--camp 2
		unit = CreateUnitByName("temple_fungal_spawn", c2+Vector(75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		self:SetTempleStats(unit,scale,125,400,5,1)
		--camp 3
		unit = CreateUnitByName("temple_fungal_slark", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_temple_cleave"):SetLevel(1)
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,150,1500,10,2)
		unit:SetModelScale(1.25)
		unit = CreateUnitByName("temple_fungal_slark", c3+Vector(0,-250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_temple_cleave"):SetLevel(1)
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,150,1500,10,2)
		unit:SetModelScale(1.25)
        ...
        --traps
        unit = CreateUnitByName("temple_missile_trap", c3+Vector(-250,-1000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(0,1,0))
        unit.trap_element = "water"
        --unit = CreateUnitByName("temple_missile_trap", c3+Vector(650,-1750,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        --unit.exp_factor = 5
        --unit.chance_for_champion = 10
        --self:SetTempleStats(unit,scale,750,100,20,0)
        --unit:SetForwardVector(Vector(0,1,0))
		...
		--camp 4
		unit = CreateUnitByName("temple_fungal_morph", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
        ...
        --traps
        unit = CreateUnitByName("temple_spike_trap", c4+Vector(-1050,-950,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)

    for i=1, horde_factor do
		--patrol
		unit = CreateUnitByName("temple_fungal_slardar", b1+Vector(0,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,350,750,20,0)
		unit:SetModelScale(1.15)
		unit = CreateUnitByName("temple_fungal_slardar", b3+Vector(0,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		...
		--camp 5
		unit = CreateUnitByName("temple_fungal_ancient", c5, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.chance_for_champion = 10
        unit:AddAbility("pve_temple_cleave"):SetLevel(1)
		...
		--camp 6
		unit = CreateUnitByName("temple_fungal_morph", c6, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_affix_frenzy"):SetLevel(1)
		unit.chance_for_champion = 20
		unit:SetModelScale(1.2)
		self:SetTempleStats(unit,scale,150,450,5,1)
		unit = CreateUnitByName("temple_fungal_morph", c6+Vector(75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--camp 7
		unit = CreateUnitByName("temple_fungal_ancient", c7, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.chance_for_champion = 10
        unit:AddAbility("pve_temple_cleave"):SetLevel(1)
		self:SetTempleStats(unit,scale,160,1500,10,1)
		unit:SetModelScale(1.0)
		unit = CreateUnitByName("temple_fungal_ancient", c7+Vector(0,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_temple_cleave"):SetLevel(1)
		self:SetTempleStats(unit,scale,160,1500,10,1)
		unit:SetModelScale(1.0)
    end
		...
		--boss 1
		unit = CreateUnitByName("temple_water_boss_1", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,200,20000,100,0)
		unit.templeboss_wetlands = 1
		unit.waterevent = 1
		--boss 2
		unit = CreateUnitByName("temple_water_boss_2", b2, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		unit.waterevent = 1
		-- boss 2.5
		unit = CreateUnitByName("temple_water_boss_5", Entities:FindByName( nil, "teleporter_3" ):GetAbsOrigin()+Vector(600,1100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,175,22500,100,0)
		unit.templeboss_wetlands = 1
		unit.waterevent = 1
		--boss 3
		unit = CreateUnitByName("temple_water_boss_3", b3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,200,20000,100,0)
		unit.templeboss_wetlands = 1
    ...
    --boss 4 wavestriker
    unit = CreateUnitByName("temple_fungal_morph_2", b3+Vector(100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    ...
    unit.templeboss_wetlands = 1
		...
		unit = CreateUnitByName("temple_water_boss_4", b4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        ...
        unit.act_specific_loot_factor = 3
		unit = CreateUnitByName("temple_water_boss_4", b4+Vector(0,275,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		unit.templeboss_wetlands = true
		COverthrowGameMode:SetTempleStats(unit,scale,75,9000,100,0)
		...
		--mass fish event
		unit = CreateUnitByName("temple_fungal_slardar", event, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,5,125,0,0)
		unit:SetModelScale(0.9)
		unit:AddAbility("fishevent"):SetLevel(1)

        --traps
        unit = CreateUnitByName("temple_missile_trap", b4+Vector(200,-1300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(1,0,0))
        ...
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,300,20,0)
        unit:SetForwardVector(Vector(-1,0,0))
        unit.trap_element = "5darts_water"]]
	end
  ...
  if zone == 12 then
    ...
    local unit
    --camp 1
    unit = CreateUnitByName("temple_jungle_raptor", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
    --unit.chance_for_champion = 10
...
for i=1, horde_factor do
    --camp 2
    unit = CreateUnitByName("temple_jungle_raptor", c2+Vector(75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    self:SetTempleStats(unit,scale,125,400,5,1)
    unit = CreateUnitByName("temple_jungle_raptor", c2+Vector(75,-75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    self:SetTempleStats(unit,scale,125,400,5,1)
    unit = CreateUnitByName("temple_jungle_raptor", c2+Vector(-75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    ...
    --camp 4
    unit = CreateUnitByName("temple_jungle_viper", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
    --unit.chance_for_champion = 100
    ...
    --camp 6
    unit = CreateUnitByName("temple_jungle_viper", c6, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 20
    unit:AddAbility("pve_affix_slow_poison"):SetLevel(1)
    self:SetTempleStats(unit,scale,500,500,10,2)
    unit:SetModelScale(0.9)
    unit = CreateUnitByName("temple_jungle_viper", c6+Vector(-100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 10
    unit:AddAbility("pve_affix_slow_poison"):SetLevel(1)
    self:SetTempleStats(unit,scale,500,500,10,2)
    unit:SetModelScale(0.9)
    ...
    unit = CreateUnitByName("temple_jungle_spider", c8+Vector(0,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 10
    self:SetTempleStats(unit,scale,350,650,7,1)
    unit:SetModelScale(0.85)
    unit = CreateUnitByName("temple_jungle_spider", c8+Vector(-50,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 10
    self:SetTempleStats(unit,scale,350,650,7,1)
    unit:SetModelScale(0.85)
    unit = CreateUnitByName("temple_jungle_spider", c8+Vector(50,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    ...
    --camp 10
    unit = CreateUnitByName("temple_jungle_spider", c10, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 10
    self:SetTempleStats(unit,scale,450,900,20,2)
    unit:SetModelScale(1.25)

    --camp 12
    unit = CreateUnitByName("temple_jungle_troll", c12, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 100
    self:SetTempleStats(unit,scale,200,2000,20,2)
    unit:SetModelScale(1.15)
    --camp 14
    unit = CreateUnitByName("temple_jungle_troll", c14, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 10
    self:SetTempleStats(unit,scale,100,1500,10,2)
    unit:SetModelScale(0.9)
    unit = CreateUnitByName("temple_jungle_troll", c14+Vector(-100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 10
    self:SetTempleStats(unit,scale,100,1500,10,2)
    unit:SetModelScale(0.9)

    --camp 16
    unit = CreateUnitByName("temple_jungle_troll", c16+Vector(75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit:SetModelScale(0.75)
    ...
    --patrol
    unit = CreateUnitByName("temple_jungle_raptor", c14+Vector(0,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 50
    unit.patrolling = zone
    self:SetTempleStats(unit,scale,350,950,20,1)
    unit:SetModelScale(1.25)
    unit = CreateUnitByName("temple_jungle_raptor", c7+Vector(0,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.chance_for_champion = 50
    unit.patrolling = zone
    ...
    --boss 1
    unit = CreateUnitByName("temple_jungle_boss_1", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    self:SetTempleStats(unit,scale,200,20000,100,0)
    unit.boss_in_act = 2

    --boss 2
    unit = CreateUnitByName("temple_jungle_boss_2", c5, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    self:SetTempleStats(unit,scale,200,20000,100,0)
    unit.boss_in_act = 3

    --boss 3
    unit = CreateUnitByName("temple_jungle_boss_3", c7, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    self:SetTempleStats(unit,scale,200,20000,100,0)
    unit.boss_in_act = 4

    --boss 4
    unit = CreateUnitByName("temple_jungle_boss_4", c9, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    self:SetTempleStats(unit,scale,200,20000,100,0)
    unit.boss_in_act = 5

    --boss 5
    unit = CreateUnitByName("temple_jungle_boss_5", c11, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    self:SetTempleStats(unit,scale,200,20000,100,0)
    unit.boss_in_act = 6

    --boss 6
    unit = CreateUnitByName("temple_jungle_boss_6", c15, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    ...
    --boss 7
    unit = CreateUnitByName("temple_jungle_boss_7", c13+Vector(450,600,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = zone
    self:SetTempleStats(unit,scale,200,20000,100,0)
    unit.boss_in_act = 8

    --egg event
    unit = CreateUnitByName("temple_dragon_egg_2", egg, true, nil, nil, DOTA_TEAM_NEUTRALS )
	...
	if zone == 4 then
		...
		local unit
		--camp 1                 
		unit = CreateUnitByName("temple_shadow_melee", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,25,300,20,3)
		unit = CreateUnitByName("temple_shadow_archer", c1l, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1l+Vector(0,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1l+Vector(0,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1l+Vector(0,-300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1r, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1r+Vector(0,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1r+Vector(0,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,200,5,0)
		unit = CreateUnitByName("temple_shadow_archer", c1r+Vector(0,-300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,35,200,5,0)

        --traps
        unit = CreateUnitByName("temple_missile_trap", c1+Vector(-125,-1000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(0,1,0))
		...
		--camp 2
    for i=1, horde_factor do
		unit = CreateUnitByName("temple_shadow_melee", c2+Vector(75,75,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit:SetModelScale(0.85)
		...
		--camp 3
		unit = CreateUnitByName("temple_shadow_undying", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		--unit.chance_for_champion = 10
		...
		--camp 4
		unit = CreateUnitByName("temple_shadow_undying", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,110,600,4,3)
		unit:SetModelScale(0.95)
		unit = CreateUnitByName("temple_shadow_undying", c4+Vector(-150,150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        ...
        --traps
        unit = CreateUnitByName("temple_spike_trap", c4+Vector(950,1000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)

    for i=1, horde_factor do
		--camp 5
		unit = CreateUnitByName("temple_shadow_nightspider", c5, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,300,350,10,0)
		unit = CreateUnitByName("temple_shadow_nightspider", c5+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,300,350,10,0)

		--camp 5 2
		unit = CreateUnitByName("temple_shadow_nightspider", c5+Vector(-900,-1250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,300,350,10,0)
		unit = CreateUnitByName("temple_shadow_nightspider", c5+Vector(-900,-1150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,300,350,10,0)

		--patrol
		unit = CreateUnitByName("temple_shadow_nightspider", b1+Vector(0,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,300,1250,20,0)
		unit = CreateUnitByName("temple_shadow_nightspider", b3+Vector(0,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		...
		--scale = 4.0
		--camp 6
		unit = CreateUnitByName("temple_shadow_colossus", c6+Vector(0,150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--camp 7
		unit = CreateUnitByName("temple_shadow_colossus", c7, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_temple_colossus"):SetLevel(1)
		unit:SetModelScale(1.3)
		self:SetTempleStats(unit,scale,400,500,10,1)
		unit = CreateUnitByName("temple_shadow_colossus", c7+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit:AddAbility("pve_temple_colossus"):SetLevel(1)
		unit:SetModelScale(1.3)
		self:SetTempleStats(unit,scale,400,500,10,1)

		--camp 8
		unit = CreateUnitByName("temple_shadow_demon", c8, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,150,1000,10,1)
		unit = CreateUnitByName("temple_shadow_demon", c8+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,150,1000,10,1)

		--camp 9
		unit = CreateUnitByName("temple_shadow_demon", c9, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit:SetModelScale(1.25)
        ...
        --traps
        unit = CreateUnitByName("temple_missile_trap", b2+Vector(-500,2300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        ...
        unit.random_new_direction = true
        unit = CreateUnitByName("temple_missile_trap", b2+Vector(-1000,2300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        unit.loot_hoarder_on_death_chance = 10
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        ...
        unit.trap_element = "shadowshort"
        unit.random_new_direction = true
        unit = CreateUnitByName("temple_missile_trap", b2+Vector(-750,1800,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        unit.loot_hoarder_on_death_chance = 10
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
		...
		--scale = 3.0
		--boss 1
		unit = CreateUnitByName("temple_shadow_boss_1", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,200,20000,100,0)
		unit.templeboss_shadowlands = 1
        unit:SetForwardVector(Vector(-1,1,0))
		--boss 2
		unit = CreateUnitByName("temple_shadow_boss_2", b2, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,175,25000,100,0)
		unit.templeboss_shadowlands = 1
		--boss 3
		unit = CreateUnitByName("temple_shadow_boss_3", b3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		--scale = 4.0
		--boss 4
		unit = CreateUnitByName("temple_shadow_boss_4", b4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        ...
        unit:SetForwardVector(Vector(0,-1,0))
		--boss 5
		unit = CreateUnitByName("temple_shadow_boss_5", b5, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        SetSwarmOnDeath(unit, 3, 3, Vector(0,2000,0), zone, "temple_shadow_colossus", nil, false)
		self:SetTempleStats(unit,scale,150,25000,100,0)
		unit.templeboss_shadowlands = 1
		--boss 6
		unit = CreateUnitByName("temple_shadow_boss_6", b6, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		tab.Duration = -1
    for i=1, horde_factor do
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(250,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,115,2500,0,0)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(-250,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,115,2500,0,0)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(-250,-250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,115,2500,0,0)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(250,250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,115,2500,0,0)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(0,250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,115,2500,0,0)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(0,250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,115,2500,0,0)
		...
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(-150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        unit.templeboss_shadowlands = 1
		self:SetTempleStats(unit,scale,115,6000,100,0)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_shadow_gravemaker", event+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        unit.templeboss_shadowlands = 1
	...
	if zone == 8 then
		...
		--bosses
		unit = CreateUnitByName("temple_volcano_boss_1", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.bosslikescaling = true
    ...
    for i=1, horde_factor do
        --warden
        unit = CreateUnitByName("temple_volcano_fireele", b1+Vector(-250,900,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.chance_for_champion = 25
		...
		--bosses
		unit = CreateUnitByName("temple_throne_boss_1", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
    ...
    for i=1, horde_factor do
		--camp 1                 
		unit = CreateUnitByName("temple_tomb_skeleton", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
        SetSwarmOnDeath(unit, 5, 15, Vector(-2000,-2000,0), 4, nil, nil, false)
		--unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,350,1250,20,3)

		--camp 2
		unit = CreateUnitByName("temple_tomb_bug", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,150,750,7,1)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_tomb_bug", c3+Vector(200,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,150,750,7,1)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_tomb_bug", c3+Vector(-200,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,150,750,7,1)
		...
		--camp 3
		unit = CreateUnitByName("temple_tomb_spider", c5, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 20
		self:SetTempleStats(unit,scale,225,500,10,2)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
		unit = CreateUnitByName("temple_tomb_spider", c5+Vector(225,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,225,500,10,2)
		unit:FindAbilityByName("pveboss_system"):ApplyDataDrivenModifier(unit, unit, "modifier_invisible", tab)
        local unit_pointer = unit
        ...
        --traps
        unit = CreateUnitByName("temple_spike_trap", c3+Vector(-650,350,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)

        --traps
        unit = CreateUnitByName("temple_missile_trap", c4+Vector(675,700,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(-1,-1,0))
		...
		--bosses
		unit = CreateUnitByName("temple_tomb_boss_1", c2, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		unit = CreateUnitByName("temple_tomb_boss_2", c4+Vector(0,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		unit.ischallengeboss = 1
        unit:SetForwardVector(Vector(1,-1,0))
		unit = CreateUnitByName("temple_tomb_boss_3", c6, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
	...
	if zone == 5 then
		...
		local unit
		--camp 1   
		unit = CreateUnitByName("temple_dragon_jakiro", c1+Vector(150,-500,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		...
		--patrol
		unit = CreateUnitByName("temple_dragon_jakiro", c1+Vector(150,-400,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		...
		--camp 2
		unit = CreateUnitByName("temple_dragon_jakiro", c2l, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,1)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2l+Vector(0,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,0)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2l+Vector(0,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,0)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2l+Vector(0,-300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,0)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2r, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,1)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2r+Vector(0,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,0)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2r+Vector(0,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,0)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		unit = CreateUnitByName("temple_dragon_jakiro", c2r+Vector(0,-300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,35,250,5,0)
		unit:AddAbility("pve_aa_root"):SetLevel(1)

		--camp 3
    for i=1, horde_factor do
		unit = CreateUnitByName("temple_dragon_jakiro", c3+Vector(475,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit:SetModelScale(1.1)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
		self:SetTempleStats(unit,scale,125,1000,10,1)
        unit.loot_hoarder_on_death_chance = 25
		unit = CreateUnitByName("temple_dragon_melee", c3+Vector(475,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		--unit:SetModelScale(1.1)
		self:SetTempleStats(unit,scale,125,1000,10,1)
		unit:AddAbility("pve_aa_root"):SetLevel(1)
        ...
        --traps
        unit = CreateUnitByName("temple_missile_trap", c3+Vector(150,1500,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(0,-1,0))
        unit.trap_element = "ice"
        unit = CreateUnitByName("temple_missile_trap", c3+Vector(-50,1225,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(-1,0,0))
        unit.trap_element = "ice"
        unit = CreateUnitByName("temple_missile_trap", c10+Vector(2800,-1000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(0,1,0))
    ...
    for i=1, horde_factor do
		--camp 4
		unit = CreateUnitByName("temple_dragon_melee", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		--unit.chance_for_champion = 10
		...
		--camp 5
		unit = CreateUnitByName("temple_dragon_garg", c5+Vector(100,250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		...
		--camp 6
		unit = CreateUnitByName("temple_dragon_garg", c6+Vector(-3700,1875,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,200,250,7,1)
		unit = CreateUnitByName("temple_dragon_garg", c6+Vector(-3850,1800,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,200,250,7,1)
		unit = CreateUnitByName("temple_dragon_garg", c6+Vector(-3550,1800,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,200,250,7,1)

		--scale = 7.0

		--camp 8
		unit = CreateUnitByName("temple_dragon_garg2", c8, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 20
		...
		--camp 7
		unit = CreateUnitByName("temple_dragon_garg2", c7, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit:SetModelScale(1.05)
		self:SetTempleStats(unit,scale,160,1250,10,1)
		unit = CreateUnitByName("temple_dragon_garg2", c7+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,160,1250,10,1)
		unit:SetModelScale(1.05)
		...
		--camp 9
		unit = CreateUnitByName("temple_dragon_garg2", c9+Vector(100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125,150,4,0)
		unit = CreateUnitByName("temple_dragon_garg2", c9+Vector(-100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125,150,4,0)
		unit = CreateUnitByName("temple_dragon_garg2", c9+Vector(50,50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125,150,4,2)
		unit = CreateUnitByName("temple_dragon_garg2", c9+Vector(-50,50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125,150,4,0)
		unit = CreateUnitByName("temple_dragon_melee", c9+Vector(0,-50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,125,750,4,0)

		--camp 10
		unit = CreateUnitByName("temple_dragon_garg2", c10, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		...
		--boss 1
		unit = CreateUnitByName("temple_dragon_boss_1", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,200,20000,100,0)
		unit.templeboss_dragon = 1
        unit:SetForwardVector(Vector(0,-1,0))
		...
		--boss 2
		unit = CreateUnitByName("temple_dragon_boss_2", b2+Vector(300,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		--boss 3
		unit = CreateUnitByName("temple_dragon_boss_3", b3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
        ...
        --boss 7
        unit = CreateUnitByName("temple_dragon_boss_7", b7, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.act = zone
        self:SetTempleStats(unit,scale,200,20000,100,0)
        unit.templeboss_dragon = 1
        unit:SetForwardVector(Vector(0,1,0))
		...
		--boss 4
		unit = CreateUnitByName("temple_dragon_boss_4_depressive", b4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,150,12500,100,0)
		unit.templeboss_dragon = 1
		unit.templeboss_t1setquest = 1
        unit:SetForwardVector(Vector(0,-1,0))
        unit.act_specific_loot_factor = 3
		local dc1 = unit
		unit = CreateUnitByName("temple_dragon_boss_4_angry", b4+Vector(-250,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,75,12500,100,0)
		unit.templeboss_dragon = 1
		unit.templeboss_t1setquest = 1
        unit:SetForwardVector(Vector(0,-1,0))
		local dc2 = unit
		unit = CreateUnitByName("temple_dragon_boss_4_supportive", b4+Vector(250,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,75,12500,100,0)
		unit.templeboss_dragon = 1
		unit.templeboss_t1setquest = 1
    ...
    elseif v1 <= 667 then
      ...
      unit = CreateUnitByName("temple_dragon_egg_2", random_pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
    ...
    for i=1, horde_factor do
		--camp 1   
		unit = CreateUnitByName("temple_mount_fireele", c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,300,1500,20,3)

		--camp 2
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,40,250,5,1)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(75,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,0)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(50,-200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,0)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(25,-300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,1)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(0,-400,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,1)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(125,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,0)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(150,200,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,0)
		unit = CreateUnitByName("temple_mount_archer", c2+Vector(175,300,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,40,250,5,1)

		--scale = 15

		--camp 3
		unit = CreateUnitByName("temple_mount_fireele", c3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		self:SetTempleStats(unit,scale,60,1000,10,1)
		unit = CreateUnitByName("temple_mount_archer", c3+Vector(100,150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
        ...
        --traps
        unit = CreateUnitByName("temple_missile_trap", c3+Vector(-300,600,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        --unit.chance_for_champion = 10
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit:SetForwardVector(Vector(0,1,0))
		...
		--patrol
		unit = CreateUnitByName("temple_mount_archer", b1+Vector(100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,125,350,7,1)
		unit = CreateUnitByName("temple_mount_archer", b1+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,125,350,7,1)
		unit = CreateUnitByName("temple_mount_archer", b1+Vector(200,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.patrolling = zone
		self:SetTempleStats(unit,scale,125,350,7,1)
		...
		--camp 4
		unit = CreateUnitByName("temple_mount_tree", c4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		...
		local tab = {}
		tab.Duration = -1
		unit = CreateUnitByName("temple_mount_tree", c4+Vector(-100,1500,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		...
		--camp 5
		unit = CreateUnitByName("temple_mount_tree", c5+Vector(100,250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		...
		--camp 6
		unit = CreateUnitByName("temple_mount_plant", c6+Vector(75,100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,200,250,7,1)
		unit = CreateUnitByName("temple_mount_plant", c6+Vector(75,-100,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,200,250,7,1)
		unit = CreateUnitByName("temple_mount_plant", c6, true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,200,250,7,1)

		--camp 7
		unit = CreateUnitByName("temple_mount_plant", c7, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 10
		unit:SetModelScale(1.05)
		self:SetTempleStats(unit,scale,250,750,10,2)
		unit = CreateUnitByName("temple_mount_plant", c7+Vector(150,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		--unit.chance_for_champion = 10
		...
		--camp 8
		unit = CreateUnitByName("temple_mount_wisp", c8, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.chance_for_champion = 20
		unit:SetModelScale(1.35)
		self:SetTempleStats(unit,scale,325,1250,20,4)

		--camp 9
		unit = CreateUnitByName("temple_mount_wisp", c9+Vector(100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,120,150,4,1)
		unit = CreateUnitByName("temple_mount_wisp", c9+Vector(-100,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,120,150,4,1)
		unit = CreateUnitByName("temple_mount_wisp", c9+Vector(50,50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,120,150,4,1)
		unit = CreateUnitByName("temple_mount_wisp", c9+Vector(-50,50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,120,150,4,1)
		unit = CreateUnitByName("temple_mount_wisp", c9+Vector(0,-50,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
		self:SetTempleStats(unit,scale,120,150,4,1)
    end
		--scale = 12.5

		--boss 1
		unit = CreateUnitByName("temple_mount_boss_1", b1, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		--boss 2
		unit = CreateUnitByName("temple_mount_boss_2", b2, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,200,20000,100,0)
		unit.templeboss_mount = 1
		...
		--boss 3
		unit = CreateUnitByName("temple_mount_boss_3", b3, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		...
		--boss 4
		unit = CreateUnitByName("temple_mount_boss_4", b4, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,175,22500,100,0)
		unit.templeboss_mount = 1
		...
		--boss 5
		unit = CreateUnitByName("temple_mount_boss_5", b5, true, nil, nil, DOTA_TEAM_NEUTRALS )
		unit.act = zone
		self:SetTempleStats(unit,scale,175,22500,100,0)
		unit.templeboss_mount = 1
		unit:SetForwardVector(Vector(0,-1,0))
...
function COverthrowGameMode:SpawnDragonEggs()
	...
	local scale = self.jungledifficulty * 1.0
	local event = Entities:FindByName( nil, "eggevent" ):GetAbsOrigin()+Vector(0,-200,0)
	local unit = CreateUnitByName("temple_dragon_egg", event+Vector(175,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
	self:SetTempleStats(unit,scale,0,250,0,0)
	unit:SetModelScale(1.1)
	unit:AddAbility("pve_temple_dragon_hatch"):SetLevel(1)
	unit = CreateUnitByName("temple_dragon_egg", event+Vector(-175,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
	self:SetTempleStats(unit,scale,0,250,0,0)
	unit:SetModelScale(1.1)
	unit:AddAbility("pve_temple_dragon_hatch"):SetLevel(1)
	unit = CreateUnitByName("temple_dragon_egg", event+Vector(0,175,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
	self:SetTempleStats(unit,scale,0,250,0,0)
	unit:SetModelScale(1.1)
	unit:AddAbility("pve_temple_dragon_hatch"):SetLevel(1)
	unit = CreateUnitByName("temple_dragon_egg", event+Vector(-90,-150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
	self:SetTempleStats(unit,scale,0,250,0,0)
	unit:SetModelScale(1.1)
	unit:AddAbility("pve_temple_dragon_hatch"):SetLevel(1)
	unit = CreateUnitByName("temple_dragon_egg", event+Vector(90,-150,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
...
function FishEvent(event)
    ...
    for i=1, 35 do
        ...
        Timers:CreateTimer(i*0.75,function() 
           ...
           local unit = CreateUnitByName("temple_fungal_slardar", eventpos+offset, true, nil, nil, DOTA_TEAM_NEUTRALS )
           if i == 35 then
...
function RaptorHatchEvent(event)
    ...
    local unit = CreateUnitByName("temple_jungle_lizard", eventpos, true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.canleavespawnpos = true
    ...
    for i=1, 1 do
      --print("here " .. tostring(i))
        Timers:CreateTimer(1.0,function() 
          ...
          --local offset = RandomVector(1):Normalized()*math.random(150,900)
          local unit = CreateUnitByName("temple_jungle_boss_0", eventpos, true, nil, nil, DOTA_TEAM_NEUTRALS )
          unit.canleavespawnpos = true
...
function VolcanoEvent1(event)
    ...
    local ab
    for i=1,2 do
        unit = CreateUnitByName("temple_volcano_fireborn", pos[4]+Vector(-1050+i*700, 0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
    ...
    for i=1, 35 do
     ...
     Timers:CreateTimer(i*1.25+extra,function() 
           ...
           --local unit
           if i == 35 then
              unit = CreateUnitByName("temple_volcano_fireborn", spawn_pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
              unit:AddAbility("pve_event_volcano2")
              ...
              unit = CreateUnitByName("npc_dota_creature_imp_volcano", spawn_pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
              unit.noexp = 1
              unit.noinvisaffix = true
...
function VolcanoEvent2(event)
	...
	for i=1, 4 do
     ...
     Timers:CreateTimer(delay+0*i+extra,function() 
      		...
      		local offset = RandomVector(1):Normalized()*50
           local unit
           unit = CreateUnitByName("temple_volcano_flameborn", pos[i%4]+offset, true, nil, nil, DOTA_TEAM_NEUTRALS )
           unit.noinvisaffix = true
           unit.can_have_affixes = true
...
function VolcanoEvent3(event)
	...
	for i=1, 1 do
     ...
     Timers:CreateTimer(i*0.9,function() 
      		...
      		local offset = RandomVector(1):Normalized()*50
           local unit
           unit = CreateUnitByName("temple_volcano_boss_2", pos[4]+offset, true, nil, nil, DOTA_TEAM_NEUTRALS )
           unit.bosslikescaling = true
...
function VolcanoEvent4(event)
	...
	for i=1, 6 do
		--print("here " .. tostring(i))
     Timers:CreateTimer(i*18-15,function() 
      		--print("here inside " .. tostring(i))
      		--print(eventpos)
      		local offset = RandomVector(1):Normalized()*50
           local unit
           unit = CreateUnitByName("npc_dota_creature_imp_volcano", pos[i%4]+offset, true, nil, nil, DOTA_TEAM_NEUTRALS )
           unit:AddAbility("pve_temple_meteor_low"):SetLevel(1)
           unit.act = 8
           unit.noinvisaffix = true
          ...
          unit.bosslikescaling = true
	        --end
           COverthrowGameMode:SetTempleStats(unit,scale,100+i*35,750,100,0)
           unit:SetModelScale(1.3)
           local tab = {}
           tab.Duration = 0.1
           local ab = unit:FindAbilityByName("pveboss_system")
...
function BurningTreeEvent(event)
     ...
     Timers:CreateTimer(i*0.75+extra,function() 
      		...
      		if roll == 1 then
               ...
               if i == 1 then
                ...
                local unit = CreateUnitByName(unitname, bosstargetpos+Vector(math.random(1,200),math.random(1,200),0), true, nil, nil, DOTA_TEAM_NEUTRALS )
                unit.noexp = 1
                if i < 58 then
                    if i == 1 or i == 28 then
                      if roll == 3 then
			...
			if i >= 58 then
				unit.canleavespawnpos = true
				if roll == 3 then
					COverthrowGameMode:SetTempleStats(unit,scale,100,4500,100,2)
					unit:SetModelScale(1.3)
                  elseif roll == 2 then
                     COverthrowGameMode:SetTempleStats(unit,scale,150,2500,100,2)
                     unit:SetModelScale(1.3)
                 else
                     COverthrowGameMode:SetTempleStats(unit,scale,125,3500,100,2)
                     unit:SetModelScale(1.1)
                 end
                 unit.act = 6
                 unit.templeboss_mount = 1
 ...
 Timers:CreateTimer(90,function()
     ...
     unit.act_specific_loot_factor = 3
     unit.act = 6
     COverthrowGameMode:SetTempleStats(unit,scale,100,15000,100,0)
     unit.templeboss_mount = 1
	...
	if name == "temple_sand_dustboar" then
		if math.random(1,100) <= 50 then
			...
			for i=1, 4 do
				...
				unit = CreateUnitByName(name, center + vec, true, nil, nil, DOTA_TEAM_NEUTRALS )
...
function COverthrowGameMode:ReportBossKills(kills)
	local score = math.floor(kills*COverthrowGameMode.jungledifficulty+0.5)
...
function COverthrowGameMode:OnPlayerLevelUp(keys)
    ...
    if caster.temple_class and not caster.dont_add_temple_class_ability and caster:GetLevel() % 5 == 0 then
...
function COverthrowGameMode:SetTempleStats(target, scale, dmg, hp, reward, affixes)
	...
	if (target.chance_for_champion and scale > 1.0) then
		...
		if math.random(1,100) <= target.chance_for_champion then
			is_champion = true
			reward = 100
            if not target.isboss then
			     target.nobosskill = 1
            end
			local champion_power = math.random(3,6)
	...
	if target.chance_for_mogushan_affixes then
		if math.random(1,100) <= target.chance_for_mogushan_affixes then
			...
			--max hp x 10 - 19
			hp = hp * (1 + 0.1 * champion_power)
			--max dmg x 2-2.75
			dmg = dmg * (1 + 0.1 * champion_power)
		end
	end

	--hp = hp * 1.25
	--act 1 upscale
	--if self.jungledifficulty > 1.0 and target.templeboss_wolf then
	--	hp = hp * 2.5
	--	dmg = dmg * 2.0
	--end
	--if isboss then
	...
	--end
		...
		--patch 2.8, exponential
	if not isboss then
		hp = exp_scale(scale, hp, 0.25, 0)
		--hp = hp * 0.35
	else
		hp = exp_scale(scale, hp, 0.25, 0)
	end
	
	if not target:HasModifier("modifier_loot_goblin") then
		hp = hp * playerscale
	end

    hp = hp * 1.2 --cuz longer global cd, 5.4 patch

    --first play experience adjustments
    local weaken = 1.0
    if self.jungledifficulty <= 1.0 and (isboss or is_champion) then
        weaken = 0.25 + scale - 1
    end
    if weaken > 1.0 then
        weaken = 1
    ...
    dmg = dmg * playerscale
    target:SetBaseDamageMin(dmg*scale*weaken*weaken_on_500)
    target:SetBaseDamageMax(dmg*scale*weaken*weaken_on_500)
...
function COverthrowGameMode:CreateAct11LastBoss()
    ...
    Timers:CreateTimer(26,function()
        ...
        unit = CreateUnitByName("temple_temple_boss_1", center, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.act = 11
...
function COverthrowGameMode:CreateActBoss(act, boss_unlock)
	...
	if not self.act11_bosses_spawned then
		...
		for i=1,1*horde_factor do
			local unit = CreateUnitByName("temple_shadow_spectre_2", creep1+Vector(0,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
			unit:SetModelScale(1.1)
			unit:AddAbility("pve_temple_ambush")
		...
		for i=1,3*horde_factor do
			local unit = CreateUnitByName("temple_shadow_spectre_2", creep2+Vector(-300+150*i,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
			unit:SetModelScale(0.85)
			unit:AddAbility("pve_temple_ambush")
			unit:AddAbility("temple_shadowstrike")
		...
		for i=1,1*horde_factor do
			local unit = CreateUnitByName("temple_shadow_spectre_2", creep3+Vector(0,400,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
			unit:SetModelScale(1.0)
			unit:AddAbility("pve_temple_ambush")
			unit:AddAbility("temple_shadowstrike")
		...
		for i=1,horde_factor do
			local unit = CreateUnitByName("temple_shadow_archer", creep3+Vector(-150+150*i,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
            unit.chance_for_champion = 100
        ...
        --traps
        local unit = CreateUnitByName("temple_spike_trap", creep1+Vector(150,1000,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)
        unit = CreateUnitByName("temple_spike_trap", creep3+Vector(-950,-115,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)
...
function CinematicApproachMessage(event)
    ...
    if id and id == 19 and not target.cinematic_message_triggered then
        ...
        text = "Qrog, Titan of Power: I'm a natural disaster!"
    end
    ...
    if id and id == 21 and not target.cinematic_message_triggered and (event.target:GetAbsOrigin()-target:GetAbsOrigin()):Length() <= 600 then
        ...
        Timers:CreateTimer(7.5,function()
            ...
            Timers:CreateTimer(20.0,function()
                unit = CreateUnitByName("temple_chest", tree_position+Vector(-450,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
                unit:SetModelScale(2)
                ...
                unit = CreateUnitByName("temple_chest", tree_position+Vector(-575,125,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
                unit:SetModelScale(2)
...
function MogushanSpawnRound( scale, zone )
    ...
    if not COverthrowGameMode.mogushan_round then
        COverthrowGameMode.mogushan_round = 1
    else
    ...
    local initial_monster_formation = math.random(1,3)

    --round 21 to 30 = new bosses
    local titan_father_round = false
    if COverthrowGameMode.mogushan_round >= 21 then
        initial_monster_formation = math.random(1,2)
    ...
    if initial_monster_formation == 1 then
        ...
        local unit = CreateUnitByName(boss, c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
        ...
        if COverthrowGameMode.mogushan_round == last_round then
            unit.drop_chance_factor = 500
        end
        unit.act = zone
        COverthrowGameMode:SetTempleStats(unit,scale,100,15000,100,titan_father_affixes)
        unit:SetForwardVector(Vector(0,-1,0))
        unit.templeboss_mogushan = 1
        --set event at 50
        Timers:CreateTimer(1,function()
        ...
        unit = CreateUnitByName(champ, c1+Vector(250,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.chance_for_champion = 100
        ...
        unit:SetForwardVector(Vector(0,-1,0))
        if COverthrowGameMode.mogushan_round == last_round then
            unit = CreateUnitByName(champ, c1+Vector(0,-250,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
            unit.chance_for_champion = 100
            unit.champion_affixes_max = 2
            COverthrowGameMode:SetTempleStats(unit,scale,50,750,0,0)
            unit:SetForwardVector(Vector(0,-1,0))
    ...
    if initial_monster_formation == 2 then
        ...
        if COverthrowGameMode.mogushan_round == 20 then
            unit.drop_chance_factor = 250
        end
        unit.act = zone
        COverthrowGameMode:SetTempleStats(unit,scale,150,20000,100,titan_father_affixes)
        unit:SetForwardVector(Vector(0,-1,0))
        unit.templeboss_mogushan = 1
        --set event at 50
        Timers:CreateTimer(1,function()
        ...
        Timers:CreateTimer(1,function()
            if boss_pointer and not boss_pointer:IsNull() then
                if boss_pointer:GetHealth() / boss_pointer:GetMaxHealth() <= 0.9 then
                    MogushanBattleEvent( boss_pointer, act )
                else
                    return 1
                end
        ...
        if COverthrowGameMode.mogushan_round == last_round then
            ...
            Timers:CreateTimer(1,function()
                if boss_pointer and not boss_pointer:IsNull() then
                    if boss_pointer:GetHealth() / boss_pointer:GetMaxHealth() <= 0.2 then
                        MogushanBattleEvent( boss_pointer, act )
                    else
                        return 1
                    end
    ...
    if initial_monster_formation == 3 then
        --3 champs
        local champ = GetRandomMonsterNameByAct(act)
        local unit = CreateUnitByName(champ, c1, true, nil, nil, DOTA_TEAM_NEUTRALS )
        local boss_pointer = unit
        COverthrowGameMode.current_mogushan_arena_boss = unit
        if act == 9 then
            unit.act_specific_loot_factor = 3
        ...
        unit.templeboss_mogushan = 1
        unit.isboss = 1
        COverthrowGameMode:SetTempleStats(unit,scale,100,1250,100,0)
        unit:SetForwardVector(Vector(0,-1,0))
        --set event at 50
        Timers:CreateTimer(1,function()
            if boss_pointer and not boss_pointer:IsNull() then
        ...
        unit.chance_for_champion = 100
        unit.champion_affixes_max = 2
        COverthrowGameMode:SetTempleStats(unit,scale,50,750,20,0)
        unit:SetForwardVector(Vector(0,-1,0))
        --champ = GetRandomMonsterNameByAct(act) --3rd same as 2nd
        unit = CreateUnitByName(champ, c1+Vector(-250,0,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.act = zone
        unit.chance_for_champion = 100
        unit.champion_affixes_max = 2
...
function CreateAct11MergeBoss( boss1, boss2, delay )
    ...
    Timers:CreateTimer(delay,function()
        ...
        local unit = CreateUnitByName(name, center, true, nil, nil, DOTA_TEAM_NEUTRALS )
        unit.act = boss1
...
function COverthrowGameMode:CreateAct7Omni()
    ...
    local unit = CreateUnitByName("temple_tomb_boss_5", Entities:FindByName( nil, "cathedral" ):GetAbsOrigin(), true, nil, nil, DOTA_TEAM_NEUTRALS )
    unit.act = 7
...
function QuestEscort1( state )
    ...
    if distance_to_goal <= 250 then
        ...
        Timers:CreateTimer(5.0,function()
            local unit = CreateUnitByName("temple_chest", endpos+Vector(0,-500,0), true, nil, nil, DOTA_TEAM_NEUTRALS )
            unit:SetModelScale(2)
    ...
    if state and state > 0 then
        ...
        if state == 6 and distance_to_goal < total_distance * 0.57 then
            state = state + 1
        ...
        if state == 11 and distance_to_goal < total_distance * 0.39 then
            ...
            unit = CreateUnitByName("temple_wolf_boss_6", pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
            unit.act = 12
            unit.canleavespawnpos = true
...
function Act13Event(event)
    ...
    for i=1, 100 do
        ...
        Timers:CreateTimer(i*4.0+delay,function()
            local unitname = GetRandomMonsterNameByAct(13)
            local dmg = 75
            local hp = 1000
            local max = 1
            if i % 10 == 1 then -- every 10th 10er skeletons
                max = 10
                unitname = "act13_6"
            end
            if i == 40 then -- drow
            ...
            if i == 41 then -- catapults
                max = 1
                unitname = "act13_10"
            end
            if i == 70 then -- legionnairs
                max = 2
                unitname = "act13_9"
            end
            if i == 71 then -- catapults
                max = 3
                unitname = "act13_10"
            end
            if i == 72 then -- catapults
                max = 3
                unitname = "act13_10"
            end
            if i == 100 then -- lich
            ...
            for j=1,max do
                local unit = CreateUnitByName(unitname, spawnpos, true, nil, nil, DOTA_TEAM_NEUTRALS )
                if i % 10 == 0 and j == 1 and i ~= 40 and i ~= 41 and i ~= 70 and i ~= 71 and i ~= 72 and i ~= 100 then --every 10th a champ
                    unit.chance_for_champion = 100
                    unit.act = 13
                end
                local loot = 0
...
function ReviveUnit(unit, killer)
    ...
    Timers:CreateTimer(2.5, function()
        local new_unit = CreateUnitByName(name, pos, true, nil, nil, DOTA_TEAM_NEUTRALS )
...
COverthrowGameMode.ClassQuestData = {
    ...
    {"Defend the Wall", "Clear Act 13 on Monster Level 50 or higher without your team losing a life.", {"npc_dota_hero_drow_ranger", "npc_dota_hero_crystal_maiden", "npc_dota_hero_sven"}},
    ...
    {"Haunting the Dead", "Clear Act 7 on Monster Level 50 or higher in less than 8 Minutes without your team losing a life.", {"npc_dota_hero_abaddon", "npc_dota_hero_pugna", "npc_dota_hero_phantom_lancer", "npc_dota_hero_chen", "npc_dota_hero_pudge"}},
    ...
    {"Shadowland Assault", "Clear Act 4 on Monster Level 50 or higher in less than 10 Minutes without your team losing a life.", {"npc_dota_hero_phantom_assassin", "npc_dota_hero_riki", "npc_dota_hero_warlock", "npc_dota_hero_slark"}},

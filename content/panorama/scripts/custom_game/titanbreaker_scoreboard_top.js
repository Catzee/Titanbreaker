...
function _ScoreboardUpdater_UpdatePlayerPanel(scoreboardConfig, playersContainer, playerId, localPlayerTeamId) {
    ...
    if (playerInfo) {
        ...
        let playerRespawnTime = playerInfo.player_respawn_seconds + 1;
        let isRip = false;

        if(playerRespawnTime > 10) {
            playerRespawnTime = "∞";
            isRip = true;
        }

        playerPanel.FindChildInLayoutFile("RespawnIndicator").SetHasClass("Rip", isRip);

        _ScoreboardUpdater_SetTextSafe(playerPanel, "RespawnTimer", playerRespawnTime); // value is rounded down so just add one for rounded-up
...
(function () {
    ...
    var scoreboardConfig = {
        "teamXmlName": "file://{resources}/layout/custom_game/titanbreaker_scoreboard_top_team.xml",
        "playerXmlName": "file://{resources}/layout/custom_game/titanbreaker_scoreboard_top_player.xml",
